...
public class Facepp {
    ...
    public FaceppConfig getFaceppConfig() {
        ...
        faceppConfig.is_smooth =((int)configs[10]==0?false:true);
        faceppConfig.is_back_camera=((int)configs[11]==0?false:true);
        return faceppConfig;
    }
    ...
    public void setFaceppConfig(FaceppConfig faceppConfig) {
        ...
        NativeFaceppAPI.nativeSetFaceppConfig(FaceppHandle, faceppConfig.minFaceSize, faceppConfig.rotation,
                ...
                faceppConfig.roi_right, faceppConfig.roi_bottom, faceppConfig.face_confidence_filter,faceppConfig.one_face_tracking,faceppConfig.is_smooth,
                faceppConfig.is_back_camera,faceppConfig.is_smooth_track,faceppConfig.is_rect_Qualcomm,faceppConfig.happy_confidence_filter,faceppConfig.happy_threshold);
    }
    ...
    public static class FaceppConfig {
        ...
        public boolean is_smooth =true;       ///< 获取landmark,视频流检测需要平滑，图片检测不需要
        public boolean is_back_camera=false;

        public boolean is_smooth_track=false; ///<是否开启两帧平滑
        public boolean is_rect_Qualcomm = false; ///<是否采用高通大脸框模式
        public float  happy_confidence_filter; ///<笑脸置信度阈值，低于此值的数据在连续检测的时候将被过滤掉
        public float happy_threshold; ///<笑脸连续若干帧检测为笑脸时，才返回笑脸置信度的平均值 默认为 1
