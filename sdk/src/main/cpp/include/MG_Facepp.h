...
#define MG_FPP_ATTR_ATTRIBUTE_NEW 0x50      ///< 人脸属性，新的模型将人脸属性合并一起,新建一个字段
...
typedef struct {
    ...
    MG_BOOL is_back_camera;

    MG_SINGLE work_load;					///< 定义WORK_LOAD值

    MG_BOOL  is_smooth_track;				///< 是否开启两帧平滑

	MG_BOOL  is_rect_Qualcomm;				///< 人脸框是否采用高通模式，包住额头。需要另行调用GetRect接口获取

	MG_SINGLE  happy_confidence_filter;    ///< 笑脸置信度阈值，低于此值的数据在连续检测的时候将被过滤掉

	MG_SINGLE  happy_threshold;			   ///< 笑脸连续若干帧检测为笑脸时，才返回笑脸置信度的平均值 默认为 1
